...
const PostList = observer(({ postAlls, handleLike, userId, content, setContent, handleComment }) => {
  ...
  const handleExpandComment = (commentId) => {
    setExpandedComments((prevExpanded) =>
      prevExpanded.includes(commentId) ? prevExpanded.filter((id) => id !== commentId) : [...prevExpanded, commentId],
    );
  };
      ...
      {postAlls?.map((post, index) => (
        <Card key={index} className="post-card">
          <CardHeader
            avatar={
              post.userId.avatar === '' ? (
                <Avatar className="avatar-text">{formatAccount(post.userId?.userName)}</Avatar>
              ) : (
          ...
          <CardMedia component="img" height="480" width="100" image={post?.images[0]?.url} title="Post" />
          <CardContent className="likecomment">
            <div>
              {post.likes?.includes(userId) ? (
                <GiHeartBeats className="like-icon" onClick={() => handleLike(post?._id)} />
              ) : (
                <AiOutlineHeart onClick={() => handleLike(post?._id)} />
              )}
              <Typography>{post.likes?.length} likes</Typography>
              <FaRegComment onClick={handleExpandComment.bind(null, post._id)} />
          ...
          <Collapse in={expandedComments.includes(post._id)} timeout="auto" unmountOnExit>
            {post.comments?.map((comment) => (
              <div className="comment" key={comment?._id}>
                {comment.userId?.avatar === '' ? (
                  <Avatar className="avatar-text">{formatAccount(comment.userId?.userName)}</Avatar>
                ) : (
                  <Avatar alt={comment.userId?.userName} src={comment.userId?.avatar} />
                )}
          ...
          <div className="box-comment">
            ...
            />
            <Button onClick={() => handleComment(post?._id)} disabled={!content}>
              <AiOutlineSend />
            </Button>
          </div>
        </Card>
